...
export class FluidD3 {
  constructor(node, isLinearInterpolate, state) {
    this.node = node;
    this.state = state;
    this.updating = false;
    ...
    this.lineFunction = d3.svg.line()
      ...
      .interpolate(isLinearInterpolate ? "linear-closed" : "basis-closed");
    ...
    this.path = svg.append("path").attr("d", this.lineFunction(lineData)).attr("class", this.state.className).attr('style', `fill:${state.fillGradient}`);
  ...
  draw() {
    ...
    for (var i = 0; i < roughShape.length; i += 2) {
      ...
      arcRadius = randomize(arcRadius, RND_R.arcRadius);
      var limbPoint = {
  ...
  animate() {
    ...
    setTimeout(() => this.animate(), FLOAT_ANIM_DELAY);
  }
  ...
  update(state) {
    ...
    this.path.attr("class", this.state.className).attr('style', `fill:${state.fillGradient}`).transition()
